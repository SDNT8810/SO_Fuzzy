function [states, Cost] = simulate_MFW(X_MFW, env, fis, Window_Size)
    ...
    for N = 1 : Window_Size
        ...
        FuzzySysInputs = [Preference_MF;Goal_Direction/180;dist2goal(X,X_g_sim(:,Step_Counter));X(5)/180];
        
        NJ = norm(Goal_Vector(:,Step_Counter)) + abs(Goal_Direction)/180 + (Fuzzy_Local_Direction_ref^2)/180 + (0.01/max(min(env.Lidar_Range - Points360),0.01)^2);
        J(1,Step_Counter) = NJ;
    end
